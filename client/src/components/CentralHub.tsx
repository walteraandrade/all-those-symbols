import { motion } from "framer-motion";
import { Cpu } from "lucide-react";
import { useMotion } from "@/contexts/MotionContext";

export function CentralHub() {
  const { prefersReducedMotion } = useMotion();

  const staticRingStyles = prefersReducedMotion
    ? { opacity: 0.2 }
    : undefined;

  return (
    <div
      aria-hidden="true"
      role="presentation"
      className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center justify-center z-10 pointer-events-none"
    >
      {/* Core Icon */}
      <div className="relative z-20 bg-background/50 backdrop-blur-sm rounded-full p-4 border border-primary/20 shadow-[0_0_20px_rgba(45,212,191,0.1)]">
        <Cpu className="w-6 h-6 text-primary/80" strokeWidth={1.5} />
      </div>

      {/* Breathing Halo */}
      {!prefersReducedMotion && (
        <motion.div
          className="absolute inset-0 bg-primary/5 rounded-full z-10"
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.3, 0.1, 0.3],
          }}
          transition={{
            duration: 4,
            repeat: Infinity,
            ease: "easeInOut",
          }}
        />
      )}

      {/* Inner ring */}
      {prefersReducedMotion ? (
        <div
          className="absolute w-24 h-24 border border-primary/20 rounded-full z-0"
          style={staticRingStyles}
        />
      ) : (
        <motion.div
          className="absolute w-24 h-24 border border-primary/20 rounded-full z-0"
          style={{ borderTopColor: "transparent", borderLeftColor: "transparent" }}
          animate={{ rotate: 360 }}
          transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
        />
      )}

      {/* Middle dashed ring */}
      {prefersReducedMotion ? (
        <div
          className="absolute w-32 h-32 border border-primary/10 rounded-full border-dashed z-0"
          style={staticRingStyles}
        />
      ) : (
        <motion.div
          className="absolute w-32 h-32 border border-primary/10 rounded-full border-dashed z-0"
          animate={{ rotate: -360 }}
          transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
        />
      )}

      {/* Outer ring */}
      {prefersReducedMotion ? (
        <div
          className="absolute w-48 h-48 border border-primary/5 rounded-full z-0"
          style={staticRingStyles}
        />
      ) : (
        <motion.div
          className="absolute w-48 h-48 border border-primary/5 rounded-full z-0"
          animate={{
            scale: [1, 1.05, 1],
            opacity: [0.1, 0.2, 0.1],
          }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
        />
      )}

      {/* Orbiting particles - only when motion allowed */}
      {!prefersReducedMotion && (
        <>
          <motion.div
            className="absolute w-32 h-32 z-0"
            animate={{ rotate: 360 }}
            transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
          >
            <div className="absolute top-0 left-1/2 w-1.5 h-1.5 bg-accent rounded-full shadow-[0_0_5px_var(--color-accent)] transform -translate-x-1/2 -translate-y-1/2" />
          </motion.div>

          <motion.div
            className="absolute w-20 h-20 z-0"
            animate={{ rotate: -360 }}
            transition={{ duration: 5, repeat: Infinity, ease: "linear" }}
          >
            <div className="absolute bottom-0 left-1/2 w-1 h-1 bg-primary rounded-full shadow-[0_0_5px_var(--color-primary)] transform -translate-x-1/2 translate-y-1/2" />
          </motion.div>
        </>
      )}
    </div>
  );
}
